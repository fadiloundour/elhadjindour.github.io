...
(function (root, factory) {
    ...
    if (wrapper && typeof wrapper === 'object') {
		...
		if (wrapper.children.length > 0) {
        	...
        	throw new Error("Le wrapper sélectionne ne contient aucun objet enfant.")
      	}
    }
    // The wrapper don't exist
    else {
		throw new Error("Le wrapper que vous essayez de sélectionner n'existe pas.");
    }
...
//Start de action with the main content
var scroller = new HPS('.page-wrapper', {
        ease: 0.08,
      });


/* -------- LOGOS DIANA HOMEPAGE -------- */
	const svgPathLogo = document.querySelectorAll('.logo-diana-home');
	const svgPath2 = document.querySelectorAll('.logo-name');
...
/* -------- MENU BURGER ANIMATION -------- */
	...
	var menu = document.getElementById('btn-nav');
	var home = document.getElementById('link-home');
	var about = document.getElementById('link-about');
	var portfolio = document.getElementById('link-portfolio');
	var contact = document.getElementById('link-contact');
...
/* -------- IMAGES PORTFOLIO ANIMATION -------- */
	const images = document.querySelectorAll(".content-img-work, .about-img");
	console.log(images);
...
$("input,textarea").blur(function(){
	if( $(this).val() ){
		$(this).parent().addClass("filled-form");
	} else {
		$(this).parent().removeClass("filled-form");
	}
});